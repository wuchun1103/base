...
router.beforeEach((to, from, next) => {
  ...
  if (to.path == '/login') {
    ...
    if (!jsonobj) {
      ...
      // 加载动态菜单和路由
      addDynamicMenuAndRoutes(jsonobj.loginName, to, from)
      next()
    }
  }
})
...
function handleStaticComponent(router, dynamicRoutes) {
  ...
  router.options.routes[0].children = router.options.routes[0].
  children.concat(dynamicRoutes)
}
