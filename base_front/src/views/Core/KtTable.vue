<template>
  <div>
    ...
    <el-table :data="data.content" :highlight-current-row="highlightCurrentRow" @selection-change="selectionChange"
      ...
      <el-table-column
        type="index"
        width="35" v-if="showRowIndex">
        <template slot-scope="scope" >
          <span>{{(pageRequest.pageNum - 1) * pageRequest.pageSize + scope.$index + 1}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('action.operation')"
                       width="185" fixed="right" v-if="showOperation"
      ...
      <!--<el-table-column :label="$t('action.operation')" width="185" fixed="right"-->
                       <!--v-if="showSelfBtn" header-align="center" align="center">-->
        <!--<template slot-scope="scope">-->
          <!--<kt-button icon="fa fa-edit" :label="$t('action.edit')"-->
                       <!--:size="size"-->
                     <!--@click="handleSelfClick(scope.$index, scope.row)" />-->
        <!--</template>-->
      <!--</el-table-column>-->
    </el-table>
    <!--分页栏-->
...
export default {
  ...
  methods: {
    ...
    // 编辑
    handleSelfClick: function (index, row) {
      this.$emit('handleSelfClick', {index:index, row:row})
    },
    // 删除
